...
function DocxIcon() {
  return (
    <svg
      ...
      aria-hidden="true"
      style={{ width: '19px', height: 'auto', flexShrink: 0 }}
    >
      <path
      ...
      <rect x="13" y="82" width="44" height="16" fill="#185ABD" stroke="#201F1E" strokeWidth="3" />
      <text
        x="35"
        y="94"
        textAnchor="middle"
...
function EigenpalLogo() {
  // Unique gradient ids per instance so multiple copies on a page don't
  // resolve to the first instance's fill.
  const uid = useId();
  const [g0, g1, g2] = [`${uid}-g0`, `${uid}-g1`, `${uid}-g2`];
  return (
    <svg
      viewBox="0 0 69 74"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{ width: '10px', height: '11px', flexShrink: 0 }}
      aria-hidden="true"
    >
      <path
      ...
      <defs>
        {[g0, g1, g2].map((id) => (
          ...
          >
            <stop stopColor="#2EB52E" />
            <stop offset="1" stopColor="#2EB52E" stopOpacity="0.51" />
          </linearGradient>
        ))}
      </defs>
    </svg>
